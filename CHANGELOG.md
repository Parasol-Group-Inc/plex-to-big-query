...
### Fixed
- **Corrected "Deployed" status on Open Quotes and Open RMA's** — `reports-list/sales.md` and `docs/reports/REPORT_CATALOG.md` both called these "Deployed," but neither `sales_quotes.yaml` nor `sales_returns.yaml` has ever had a Cloud Run job, scheduler, or Terraform resource, and nothing under their names exists in the GCS config bucket. The SQL and business logic are finished and schema-confirmed; the pipelines themselves were never wired up to actually run. Not fixed yet (deploying them is a separate decision) — just corrected the record so it stops claiming these produce data on a schedule when they don't.
