...
| Inventory consumption | No view or stored procedure matching "consumption" anywhere in the schema or the 14,350-row stored-procedure catalog. **New lead 2026-08-21:** a screenshot of Plex's own "Scheduled Job Requirements" UI report suggests the real Plex-native answer is a BOM-explosion-vs-inventory rollup (`Part_v_BOM`/`Part_v_Flat_BOM`/`Part_v_Job_Bom` × `Part_v_Job` × `Part_v_Container`, confirmed live) — see `reports-list/supply-chain.md`. | Get the NetSuite report definition to confirm it means the same thing as this lead before building — "consumption" could still mean material usage (jobs consuming components) vs. finished-good depletion, two different reports. |
