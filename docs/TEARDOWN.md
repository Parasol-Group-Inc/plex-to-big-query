...
>
> **The deploy guard applies here too.** Terraform runs `scripts/tf_guard.py` on every plan, apply *and destroy*, and refuses unless you are in the primary folder (`C:\F\Parasol\plex-to-big-query`), on `main`, with a clean tree equal to `origin/main`. So every `main.tf` edit below is a commit that reaches `main` (dev branch → merge → push) before you run anything, and every `terraform` command runs from the primary folder. That is deliberate: a teardown should be as visible in git as a deploy.
...
| Every `raw_*` table (dozens — one per `bq_table:` entry across all 13 `reports/*.yaml`) | ETL job (not Terraform) | Created dynamically at runtime, not tracked by Terraform. `delete_contents_on_destroy = true` (Step 2) handles this for you — no need to delete them one by one. |
...
```bash
for job in $(gcloud scheduler jobs list --location=us-central1 --project=voxdatalake \
              --format="value(name.basename())" --filter="name~plex-"); do
  gcloud scheduler jobs pause "$job" --location=us-central1 --project=voxdatalake
done
```

That pauses all 52 `plex-*` schedulers (daily and retry) and leaves the unrelated `monday-daily-sync*` ones alone.
...
Commit that change (with a `CHANGELOG.md` entry) on a `dev*` branch, merge it to `main`, push, and deploy it from the primary folder — this only updates the protection flags, nothing is deleted yet:
...
Every `raw_*` table and report view is created by the ETL jobs at runtime, not by Terraform — dozens of them across both `PlexProd` and `PlexTest` now (one per `bq_table:`/`bq_view:` entry across all 13 `reports/*.yaml`). With `delete_contents_on_destroy = true` set on both datasets in Step 2, `terraform destroy` in Step 5 drops the datasets **and everything in them** — this step is redundant in the normal case.
...
From the primary folder, on the `main` that carries Step 2's change (the guard refuses anywhere else — `deploy.sh` doesn't do destroys, so this one is a plain command):

```bash
cd C:/F/Parasol/plex-to-big-query/terraform
...
**1. Restore `main.tf`** to safe defaults (undo the one change from Step 2 — a commit to `main`, like Step 2 was):
...
**2b. Point the backend at a state bucket that exists.** `terraform/main.tf`'s `backend "gcs"` block hardcodes `bucket = "voxdatalake-terraform-state"`. If the new project uses its own state bucket, create it (outside Terraform — see DISASTER_RECOVERY.md step 2) and change that line. It is a `main.tf` change, so it reaches `main` like any other before step 3. Also note every YAML's `sql_file` hardcodes `gs://voxdatalake-report-configs/...`; a report-configs bucket with a different name needs those changed too.

**3. Follow the normal deployment steps — this recreates the full 26-job stack, not one job** (from the primary folder, on a clean pushed `main` — the deploy guard applies):
...
# Deploy again — this DOES set the image, but only because these jobs are
...
# Test each job — at minimum, every *-test variant
for job in $(grep '_ALL_JOBS:' deploy/cloudbuild.yaml | cut -d'"' -f2 | tr ' ' '\n' | grep -- '-test$'); do
  gcloud run jobs execute "$job" --region=us-central1 --project=your-new-project-id --wait
done
```
