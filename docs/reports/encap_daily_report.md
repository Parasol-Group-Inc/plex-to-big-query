...
> **Status:** ✅ Fixed and verified in both Test and Prod 2026-08-26 — manually ran `plex-etl-work-orders` (prod), confirmed `PlexProd.encap_daily_report` exists and is queryable again (0 rows, benign — see Flags below) · **Category:** Production · **Runs:** rides the Work Orders pipeline, 7:20 PM / 7:30 PM Mountain (prod/test)
...
- **Fixed 2026-09-01 — this and its 3 siblings were silently dropping real production rows.** Once actual `Part_v_Production` data finally landed on this tenant, this view (and `blending_daily_report`/`labeling_daily_report`/`packaging_daily_report`) still returned 0 — root cause was the `Part_v_Job_Op` join (`INNER JOIN`), which dropped a production row outright whenever its `Job_Op_Key` had aged out of the current `Part_v_Job_Op` extract (that table only reflects current/open operations; the production log is permanent). Changed to `LEFT JOIN` so a row is never lost this way — `job_count`/`parts_run` just go `NULL`-safe for those specific rows instead. Reverified against `PlexTest`: `packaging_daily_report`/`blending_daily_report` now show real rows (Bottling Line 1 and Preweigh 1, 2026-08-31/09-01); this report and `labeling_daily_report` still correctly show 0 — no real Encapsulation or Labeling production has been logged yet, a genuine fact now, not a join bug. See `score-card-reference/VOX_SCORECARD_PLEX_MIGRATION_MAP.md` and `CHANGELOG.md` 2026-09-01 for the full story.
...
- **Resolved 2026-08-26.** Manually ran `gcloud run jobs execute plex-etl-work-orders --region=us-central1 --project=voxdatalake --wait` (exit 0), then queried `PlexProd.encap_daily_report` directly per this repo's own rule of not trusting exit codes alone: exists, queryable, `COUNT(*) = 0` (still benign — `raw_Part_v_Job`/`raw_Part_v_Job_Op`/`raw_Part_v_Production` all confirmed still 0 rows on the real Plex prod host, so no real production data has landed yet either).
