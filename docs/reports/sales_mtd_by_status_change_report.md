...
- **⚠ Fixed 2026-09-24 — every sale read "(no rep assigned)".** `sales_rep` came from `Sales_v_Order_Salesperson` (`Sort_Order = 1`), which Vox barely uses: one row in all of test, none in prod, and that one with `Sort_Order = 0`. Vox records the rep on the **order** (`Sales_v_PO.Inside_Sales`) and the **customer** (`Common_v_Customer.Assigned_To`, on 150 of 182 customers). `sales_rep` now resolves order → customer → salesperson table, and the new **`sales_rep_source`** column (`order` / `customer` / `order_salesperson`) says which. Same resolution as the Label Design view's `bdm`. Found by the scorecard sandbox, where a full year of orders all landed on "(no rep assigned)".
- **Which sales rep owns the number.** Plex allows more than one salesperson per order, so `sales_rep_2` and the primary rep's `commission_pct` are still exposed from `Order_Salesperson` — but since Vox barely fills that table, expect them to be NULL. Whole-credit-to-the-order's-rep is what the view gives today; Jennilyn needs to say if a split is wanted.
...
- **Only a handful of real rows so far.** Deployed and verified 2026-09-04 against the 7 orders that had reached Pending Fulfillment (2073). Most test-tenant orders still sit in Pending Sales Approval (2585), so expect small or even zero counts until more real orders move through the workflow.
