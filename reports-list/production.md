...
| Encap Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign) | Built as `encap_daily_report` (`work_orders.yaml`) on `Part_v_Production`, filtered `Workcenter_Group = 'Encapsulating'`. Rebuilt same day from an initial `Part_v_Cell_Production` attempt — see verdict below. 0 rows because no real job on this tenant has run yet, confirmed via Plex UI, not a schema problem. Planned Hours/Start-Up-Stop/true attendance not built — no Plex analog found. See `spreadsheets/encap_daily_report.md`. |
| Packaging Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign), decided Bottling mapping | Built as `packaging_daily_report`, filtered `Workcenter_Group = 'Bottling'` — "Packaging" isn't a Plex workcenter group at all (it's a Department code and a separate Part_Group), but the sheet's own lines match the Bottling roster almost exactly. See `spreadsheets/packaging_daily_report.md`. |
| Labeling Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign) | Built as `labeling_daily_report`, filtered `Workcenter_Group = 'Labeling'` — direct match to the sheet's own Line 1-6 numbering. See `spreadsheets/labeling_daily_report.md`. |
| Blending Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign) | Built as `blending_daily_report`, filtered `Workcenter_Group IN ('Blending', 'Pre-Weigh')` (the sheet's grid needs both). See `spreadsheets/blending_daily_report.md`. |
