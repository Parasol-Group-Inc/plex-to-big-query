...
| Encap Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign) · **2026-09-24: scrap was always 0** (views tested `Rejected = -1`; real Plex writes 1) — fixed to `!= 0`, see `docs/SCORECARD_SANDBOX_FINDINGS.md` | Built as `encap_daily_report` (`work_orders.yaml`) on `Part_v_Production`, filtered `Workcenter_Group = 'Encapsulating'`. Rebuilt same day from an initial `Part_v_Cell_Production` attempt — see verdict below. 0 rows because no real job on this tenant has run yet, confirmed via Plex UI, not a schema problem. Planned Hours/Start-Up-Stop/true attendance not built — no Plex analog found. See `spreadsheets/encap_daily_report.md`. |
| Packaging Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign), decided Bottling mapping · **2026-09-24: scrap was always 0** (views tested `Rejected = -1`; real Plex writes 1) — fixed to `!= 0`, see `docs/SCORECARD_SANDBOX_FINDINGS.md` | Built as `packaging_daily_report`, filtered `Workcenter_Group = 'Bottling'` — "Packaging" isn't a Plex workcenter group at all (it's a Department code and a separate Part_Group), but the sheet's own lines match the Bottling roster almost exactly. See `spreadsheets/packaging_daily_report.md`. |
| Labeling Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign) · **2026-09-24: scrap was always 0** (views tested `Rejected = -1`; real Plex writes 1) — fixed to `!= 0`, see `docs/SCORECARD_SANDBOX_FINDINGS.md` | Built as `labeling_daily_report`, filtered `Workcenter_Group = 'Labeling'` — direct match to the sheet's own Line 1-6 numbering. See `spreadsheets/labeling_daily_report.md`. |
| Blending Daily Report | Google Sheet | ✅ Deployed (prod+test) 2026-08-21 (Actual + Employee + Scrap), 0 rows (benign) · **2026-09-24: scrap was always 0** (views tested `Rejected = -1`; real Plex writes 1) — fixed to `!= 0`, see `docs/SCORECARD_SANDBOX_FINDINGS.md` | Built as `blending_daily_report`, filtered `Workcenter_Group IN ('Blending', 'Pre-Weigh')` (the sheet's grid needs both). See `spreadsheets/blending_daily_report.md`. |
| Bottling Job Schedule | Google Sheet | ✅ Built 2026-08-22 | Sibling of MFG Job Schedule, bottling-specific. Built as `bottling_job_schedule_report`, 9th `bq_view` on `work_orders.yaml`, filtered `Workcenter_Group = 'Bottling'`. See `spreadsheets/bottling_job_schedule.md` |
| Weekly Production Update | Google Sheet | 🚧 Partially built 2026-08-26 | Real content received (Capacity + Goals tabs). Built `weekly_production_update_report`: Actual WTD/MTD quantity by department (Encap/Bottling/Labeling/Printing — first report to cover Printing) + % of month complete. Goal/% of Goal and the Capacity tab deliberately unbuilt (no Plex source / no confirmed formula) — see `spreadsheets/weekly_production_update.md`. Users: "Mark, Nick, Chris" |
| Rolling TAT Report | NetSuite + GSheet | ✅ Deployed — **repointed 2026-09-22, now carrying real records** · **2026-09-24: work-day TAT added** — met/missed now judged in work days, matching the standards | Same underlying concept as mapping-doc #70 "Turn Around Time Report - Rolling". Built as `quality_turnaround_time_report` (bq_view on `quality_nonconformance.yaml`). It read the classic `Quality_v_Problem` table until 2026-09-22 and was empty the whole time — Vox records on the UX screen, which writes `Quality_v_Problem_2`; repointed, and returning 22 records. It now publishes **both** clocks (`turnaround_days` from Problem_Date and `turnaround_days_from_recorded` from Recorded_Date) plus `reporting_lag_days` and `closed_on_time`, so the Problem-vs-Entered question no longer has to be answered before the report is usable — but somebody still has to say which one Vox's Performance/Bonus standards were set against. Only 2 of 22 records are closed, and both closed dates are in the future, so there is no trustworthy average yet. |
...
production quantity by production date + workcenter within each report's
`Workcenter_Group` filter.

**Corrected same day: rebuilt on `Part_v_Production`, not
`Part_v_Cell_Production`.** First deploy to test came back 0 rows on all 4;
investigating via live Plex screenshots (Job Manager, Job Detail, Job
Routing, Job Production report) showed the real cause was benign — all 16
real jobs on this tenant were freshly created that same morning with 0
actual hours logged, so of course any production-log table was empty, not
a schema/tracking-mode problem. But that investigation also surfaced a
real correction: Plex's own **"Job Production" UI report** (columns Job
No/Part No/Rev/Op No/Tracking No/Last Operation Completed/Workcenter/
**Employee**/Record Date/**Shift**/Quantity, confirmed live by Emilio)
matches `Part_v_Production` field-for-field (`Record_By`, `Report_Shift`,
`Record_Date`, `Workcenter_Key`, `Job_Op_Key`, `Quantity`, `Rejected`) —
`Part_v_Cell_Production` has no Employee/Shift/Rejected columns at all and
was the wrong table. Rebuilt all 4 views on `Part_v_Production`, which
also resolves 2 of the 3 "no Plex analog" gaps from this morning:
`employee_count`/`employees` (Record_By → Personnel_v_Employee, same
INFERRED-join pattern already used for Job_Op.Started_By/Completed_By) and
`scrap_qty` (a genuine `Rejected` flag). Still not built: Planned
Production Hours, Start-Up/Stop times, and a true Call-Outs/OFF roster —
`employee_count` shows who logged production, not who was scheduled/absent,
so it doesn't answer attendance on its own.

**Deployed to test 2026-08-21 (post-correction) — all 4 views create
cleanly, still 0 rows**, for the same benign reason: `raw_Part_v_Production`
