...
| Report | Notes |
|---|---|
| Work Orders partial buildable (labeling), Orders in Design | Native NetSuite saved searches, not individually investigated this pass (lower priority / no screenshot available) |
| Orders waiting to be completed | **Confirmed unreachable 2026-08-27** — the saved search itself has expired/been removed in NetSuite (Emilio checked live, searchid=3862 no longer resolves). Nothing to investigate — genuinely closed, not a missing-screenshot problem. |
...
| Sample Orders (`Sales \| Open Sample Orders (Pending Fulfillment)`) | **Confirmed out of scope 2026-08-26** — Emilio provided the real saved-search criteria screenshot: `Sample Order (Custom Body) = true` AND `Status IN (Partially Fulfilled, Pending Billing/Partially Fulfilled, Pending Fulfillment)` AND `Main Line = true` AND `Type = Sales Order`. `Sample Order` is a raw custom checkbox a person ticks in NetSuite — no formula, no derivation, no Plex analog at all (confirmed: `Sales_v_PO_Type` only has "Blanket"/"Spot Buy", no "Sample"). The 8 real result rows rule out the last plausible proxy too: amounts (`$39.11, $77.42, $54.32, $154.65, $105.25, $0, $0`) aren't a clean "always $0" or fixed-threshold pattern. The Status half of this search *is* Plex-buildable (`Sales_v_PO_Status` key 2073 = "Pending Fulfillment," live-confirmed) — just not paired with the Sample flag, so building "orders pending fulfillment" alone would misrepresent a different, broader report as this one. Genuinely closed, not a missing-screenshot problem. |
| Pending Certificates (`Sales \| Pending Certificates`) | **Confirmed out of scope 2026-08-26** — real saved-search criteria (screenshot): `Main Line = false` (a line-level search, not order-level) AND `Type = Sales Order` AND `Item IN (Certificate of Origin, Certificate of Free Sale)` AND `Admin Task (Custom Body) = false` AND `SO Date Released >= 11/14/2025`. Checked live against the 792 real parts already extracted in `raw_Part_v_Part` (`sales_orders` pipeline) — **zero** named or coded "Certificate": these are NetSuite-only line items, not physical parts Plex tracks at all. `Admin Task` is the same kind of raw manually-ticked custom checkbox as `Sample Order` above — no formula, no Plex correlate. Neither half of this search (the certificate line item, or the task-completion flag) has anywhere to come from in Plex. |
