...
> **2026-08-23:** while generating business-facing docs in `docs/reports/`, found that **Open Quotes** and **Open RMA's** — both listed "Deployed" in this table — have no Cloud Run job, scheduler, or Terraform resource anywhere in the repo, and nothing under their names in the GCS config bucket. The SQL and business logic are finished and schema-confirmed, but neither pipeline has ever actually been wired up to run. See [`docs/reports/sales_quotes_open_report.md`](../docs/reports/sales_quotes_open_report.md) and [`docs/reports/sales_returns_open_report.md`](../docs/reports/sales_returns_open_report.md).
...
| Open Quotes | 🛠 SQL built, decided 2026-08-21 — **not actually deployed** (see 2026-08-23 note below) | `sales_quotes_open_report` (new pipeline, `Sales_v_Quote` + `_Status`). `Open_Quote` confirmed dead (0 on all 8 statuses); built with a status-exclusion proxy. **Decided: Approved now counts as closed** (moving toward becoming an order, not still awaiting a decision) — New/Estimating/Quoted are the only "open" statuses. |
| Open RMA's | 🛠 SQL built, decided best-criteria — **not actually deployed** (see 2026-08-23 note below) | `sales_returns_open_report` (new pipeline, `Sales_v_Return` + `_Status`) — clean status-exclusion (not Closed/Cancelled). Kept 2026-08-21 — no live-data conflict to resolve. |
