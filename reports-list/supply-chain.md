...
| Purchasing \| Pending Order Requisitions: Results | NetSuite | ✅ Deployed 2026-08-22, real bug fixed 2026-08-23 | Native NetSuite search, but a Plex equivalent exists — `Purchasing_v_Requisition` + `Requisition_Status` + `Req_PO_Release`. "Shows all pending requisitions waiting to be purchased." Priority: Critical (daily use). Built as `purchasing_pending_requisitions_report` (`reports/purchasing_pending_requisitions.yaml`), own Cloud Run job + scheduler pair (9:40/9:50 PM Mountain prod/test) added via `terraform apply`. **First scheduled prod run (2026-08-23) sent a real "PARTIAL PRODUCTION" email** — the view failed to create (`STRING`/`INT64` join mismatch: `raw_Purchasing_v_Requisition` had 0 rows on both prod and test, autodetected all-STRING, while `raw_Purchasing_v_Requisition_Status` has real INT64 data). Fixed with `SAFE_CAST` on both sides of every join in the view; re-verified this time by directly querying the BigQuery view (not just checking the job's exit code, which stays 0 even when view creation fails). **Still unverified against real requisition data** — test tenant has zero real Requisition rows, so the `Item_Key` vs `Part_Key` join choice needs a recheck once real requisitions exist (prod, or a test tenant with real ones). See `docs/NETSUITE_REPORT_BUILD_PLAN.md` § Purchasing \| Pending Order Requisitions. |
| Approve Vendor Return Authorizations | NetSuite | ✅ Deployed 2026-08-23, real bug fixed same day | `Quality_v_Supplier_Return` + `_Status` + `_Type` exist and are schema-confirmed, but the expected Approving/Approved status flags are confirmed live to be unused (0 on all 6 statuses) — see `reports/quality_supplier_returns.yaml`. Built with a status-exclusion proxy (not yet Shipped/Complete/Cancelled) pending a real definition. Own Cloud Run job + scheduler pair (10:40/10:50 PM Mountain prod/test) added via `terraform apply` after being found to have zero infrastructure despite the SQL being finished. **First test run sent a "PARTIAL TEST" email** — same `STRING`/`INT64` join mismatch as the Requisitions report above (`raw_Quality_v_Supplier_Return` empty, `_Status`/`_Type` populated). Fixed with `SAFE_CAST` on both sides of every join and the status-exclusion `WHERE` filter; re-verified by directly querying the BigQuery view. |
