...
-- Differs from reports/sql/production_monthly_by_workcenter_group_view.sql only in the scrap test: `!= 0` instead of `= -1`.
...
SELECT
  ...
  SUM(IF(COALESCE(SAFE_CAST(prod.Rejected AS INT64), 0) != 0 /* PROPOSED: real Plex writes 1, not -1 */,
