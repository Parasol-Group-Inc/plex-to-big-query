...
-- Differs from reports/sql/quality_fpy_by_area_month_view.sql only in the scrap test: `!= 0` instead of `= -1`.
...
by_area_month AS (
  SELECT
    ...
    SUM(IF(COALESCE(SAFE_CAST(prod.Rejected AS INT64), 0) != 0 /* PROPOSED: real Plex writes 1, not -1 */, SAFE_CAST(prod.Quantity AS FLOAT64), 0))   AS rejected_qty,
